...
The password is dynamically generated by `sandbox-manager.ts` when starting OpenCode inside the sandbox.

## Sandbox Manager Integration

The bridge doesn't connect to an external OpenCode server. Instead, `sandbox-manager.ts`:

1. **Creates/resumes** a Vercel Sandbox per Discord channel
2. **Installs OpenCode** if not already present
3. **Starts OpenCode server** on port 4096 inside the sandbox
4. **Generates a password** for each sandbox
5. **Returns the base URL** for the OpenCode HTTP client

```typescript
const sandboxManager = getSandboxManager()
const context = await sandboxManager.getOrCreate(channelId, sandboxId, repoUrl, branch)
// context.opencodeBaseUrl = "https://sb-xxx.vercel.app"
// context.opencodePassword = "generated-password"

const runtime = new OpencodeRuntime(context.opencodeBaseUrl, context.opencodePassword)
```
